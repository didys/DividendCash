...
// Copyright (c) 2015-2018 The DividendCash developers
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, dividendcash, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all DividendCash specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
...
//    strUsage += "  -anonymizedividendcashamount=<n>     " + strprintf(_("Keep N DVD anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The DividendCash Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //DividendCash: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //DividendCash specific: zerocoin and spork DB's
                ...
                // DividendCash: load previous sessions sporks if we have them.
                ...
                // DividendCash: recalculate Accumulator Checkpoints that failed to database properly
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeDividendCashAmount = 2;
