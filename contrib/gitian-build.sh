...
VERSION=
commit=false
url=https://github.com/dividendcash-coin/DividendCash
proc=2
mem=2000
lxc=true
...
-u|--url	Specify the URL of the repository. Default is https://github.com/dividendcash-coin/DividendCash
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit DividendCash=${COMMIT} --url DividendCash=${url} ../DividendCash/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign -p $signProg --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../DividendCash/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/dividendcash-*.tar.gz build/out/src/dividendcash-*.tar.gz ../dividendcash-binaries/${VERSION}
    fi
	popd
