DividendCash Core version *3.1.0* is now available from:  <https://github.com/dividendcash-project/dividendcash/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/dividendcash-project/dividendcash/issues>


Mandatory Update
==============

DividendCash Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zDVD Staking is here! zDVD staking will be activated on the morning of the 8th of May 2018. With the release of zDVD staking, there are effectively 2 versions of zDVD, zDVD minted on the 3.0.6 DividendCash wallet or lower, and zDVD minted on DividendCash wallet version or higher. New features in this release will require the use of zDVD v2, zDVD minted on this wallet release 3.1.0 or later. If you currently hold zDVD v1 and wish to take advantage of zDVD staking and deterministic zDVD, you will need to spend the zDVD v1 to yourself and remint zDVD v2.
...
To encourage the use of zDVD and increase the DividendCash zerocoin anonymity set, the DividendCash payment schedule has been changed to the following:
...
The visual layout of the DividendCash Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zDVD minting feature. This is enabled by default and the enablezeromint=0 setting in the dividendcash.conf file will overwrite the GUI option.
...
### Summary
 ...
 The DividendCash wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the dividendcash.conf file, found in the DividendCash data directory.
...
DividendCash Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The DividendCash client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running dividendcashd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the DividendCash support group, which often times sees users say "I didn't spend that zDVD". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the DividendCash masternodes, the monthly budget available to be utilised has been increased to 42,000 DVD / month. This DVD only has the opportunity to be raised once per month (paid to winning proposals) with any unused DVD not created by the blockchain.

### Budget Finalization Fee

The DividendCash finalization fee for successful proposals has now been reduced, this fee is now 5 DVD down from 50 DVD. The total fee outlay for a successful proposal is now a total of 55 DVD.
...
-	Fixed a  bug that would not start masternodes from the DividendCash-Qt masternodes tab
-	Updated DividendCash-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and DividendCash's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zDVD Control dialog in the QT wallet, a user is able to see zDVD marked as version 1 or 2.
...
DividendCash's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which DividendCash used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to DividendCash using the bug bounty program. The modulus is now correctly imported and DividendCash's accumulators have been changed to use the new proper modulus.
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault dividendcash-cli getinfo while loading block index (Mrs-X)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/dividendcash-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
